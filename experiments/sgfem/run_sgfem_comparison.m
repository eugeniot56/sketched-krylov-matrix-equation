...
figures_folder = fullfile(results_folder, 'figures', 'sgfem');
...
switch test_problem
    ...
    case 5
        data_file = 'TP_5.mat';
    otherwise
        error('Invalid test problem. Choose 2, 3, 4 or 5.');
end
...
memory_multirb_MB = ...
    (memory_X1.bytes + memory_X2.bytes) / 1024^2;

rank_multirb = size(X1, 2);
space_dim_multirb = dimV;

memory_V = whos('V');
memory_Y = whos('Y');

memory_sketch_MB = ...
    (memory_V.bytes + memory_Y.bytes) / 1024^2;
...
exportgraphics( ...
    ...
    sprintf('residual_evolution_TP_%d.png', ...
    test_problem)), ...
    'Resolution', 300);
...
bar([time_multirb, time_sketch]);

set(gca, ...
    'XTickLabel', ...
    {'MultiRB', 'Sketched Krylov-Galerkin'});
...
exportgraphics( ...
    gcf, ...
    fullfile(figures_folder, ...
    sprintf('timing_comparison_TP_%d.png', ...
    test_problem)), ...
    'Resolution', 300);
...
summary_table = table( ...
    ["MultiRB"; "Sketched Krylov-Galerkin"], ...
    [length(error_vec); info_sketch.iterations], ...
    [space_dim_multirb; space_dim_sketch], ...
    [rank_multirb; rank_sketch], ...
    [final_err; info_sketch.final_residual], ...
    [time_multirb; time_sketch], ...
    [memory_multirb_MB; memory_sketch_MB], ...
    'VariableNames', ...
...
writetable( ...
    summary_table, ...
    fullfile(tables_folder, sprintf('sgfem_results_TP_%d.csv', test_problem)));
